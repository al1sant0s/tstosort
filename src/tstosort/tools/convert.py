...
FILE_PREFIX_STORE = "autogenerated_store_"
FILE_PREFIX_INVENTORY = "autogenerated_inventory_"
MASTERLISTS = {
...
def get_items_from_category(directory, file_prefix, category, items, objsets, lang, indent):
...
def sort_menu(storemenu, file_prefix, languages, indent, debug = False):
        ...
        for lang in languages.keys():
            ...
            for element in target_root.findall("*"):
                ...
                if element.tag == "Category":
                    ...
                    items = list()
                    items = get_items_from_category(gamescripts, file_prefix, submenu_root, items, objsets, lang, indent)
                    ...
                    for category in section.findall("Category"):
                        ...
                        items = list()
                        items = get_items_from_category(gamescripts, file_prefix, category, items, objsets, lang, indent)
...
def main():
    ...
    for i in range(len(directories)):
        ...
        for file in gamescripts.glob(f"{FILE_PREFIX_STORE}*{storemenu.stem}*"):
            os.remove(file)

        # Process the files.
        colorprint(Style.BRIGHT + Fore.GREEN, f" -> Sorting StoreMenu files from DLC: {Fore.WHITE + directory.name}", "")
        sort_menu(storemenu, FILE_PREFIX_STORE, args.language, args.indent, args.debug)

        if args.inventory is True:

            # Remove all autogenerated files.
            for file in gamescripts.glob(f"{FILE_PREFIX_INVENTORY}*{inventory.stem}"):
                os.remove(file)

            colorprint(Style.BRIGHT + Fore.GREEN, f" -> Sorting Inventory files from DLC: {Fore.WHITE + directory.name}", "")
            sort_menu(inventory, FILE_PREFIX_INVENTORY, args.language, args.indent, args.debug)
